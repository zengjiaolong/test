...
do_init(ModuleList, NewModuleList, NewStateList) ->
    ...
    case Module:init() of
        {ok, NewSate} ->            
            %% 保留该模块，收集该模块状态
			?DEBUG("do_init: module is succeed, module name=[~w]", [Module]),
            do_init(ModuleLeft, NewModuleList++[Module], NewStateList++[NewSate]);
        {ignore, Reason} ->
            ?ERROR_MSG("do_init: module is ignored, module name=[~w], reason=[~w]", [Module, Reason]),
            %% 删除该模块，不收集该模块状态
            do_init(ModuleLeft, NewModuleList, NewStateList);
        {stop, Reason} ->
            ?ERROR_MSG("do_init: module is stopped, module name=[~w], reason=[~w]", [Module, Reason]),
            %% 初始化失败
            {stop, Reason}
    end.
...
do_handle(ModuleList, StateList, NewModuleList, NewStateList, IgnoreModuleList, IgnoreStateList) ->
    ...
    case Module:handle(State) of
        ...
        {ignore, Reason} ->
            ?ERROR_MSG("do_handle: module is ignored, module name=[~w], reason=[~w]", [Module, Reason]),
            % 回调该模块的terminate方法
        ...
        {stop, Reason} ->
            ?ERROR_MSG("do_handle: module is stopped, module name=[~w], reason=[~w]", [Module, Reason]),
            % 处理失败
